...
class NatureQN(DQN):
    ...
    def initialize_models(self):
        ...
        calc_padding = lambda kernel_size, stride: ((stride - 1) * img_height - stride + kernel_size) // 2

        # 主网络 (Q-Network)
        self.q_network = nn.Sequential(
            nn.Conv2d(in_channels, 32, kernel_size=8, stride=4, padding=calc_padding(8, 4)),
            nn.ReLU(),
            nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=calc_padding(4, 2)),
            nn.ReLU(),
            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=calc_padding(3, 1)),
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(64*img_height**2, 512),
            nn.ReLU(),
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        max_target_q = torch.max(target_q_values, dim=1)[0]
        Q_samp = rewards + gamma * max_target_q * (~done_mask)
        Q_samp = Q_samp.detach()
        # 2. 从 q_values 中选出实际执行动作的Q值: Q(s, a)
        q_s_a = q_values.gather(1, actions.long().unsqueeze(1)).squeeze(1)
        # 3. 计算损失 (Loss)
